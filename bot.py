...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not user.get('captcha_passed'):
        ...
        await VerifyStates.waiting_captcha.set()
        return
    ...
    if group_id and not await is_user_in_force_group(uid):
        ...
        await message.answer(text, reply_markup=kb, parse_mode="Markdown", disable_web_page_preview=True)
        await VerifyStates.waiting_group_join.set()
        return
...
@dp.message_handler(state=VerifyStates.waiting_captcha)
...
@dp.callback_query_handler(lambda c: c.data == "verify_group_joined", state="*")
async def verify_group_joined_cb(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not await is_user_in_force_group(uid):
        try:
            await bot.answer_callback_query(callback_query.id, text="এখনও Group-এ join করেননি!", show_alert=True)
        except Exception:
            pass
...
@dp.message_handler(lambda message: message.text in ["🔙 Admin Panel", "🔙 Back to Admin"], state="*")
...
@dp.message_handler(lambda message: message.text == "🔙 Settings Menu", state="*")
...
@dp.callback_query_handler(lambda c: c.data == "main_menu_inline", state="*")
...
# ---------- User Settings ----------
@dp.message_handler(lambda message: message.text == "👤 My Settings", state="*")
...
@dp.callback_query_handler(lambda c: c.data == "user_settings_toggle_auto")
...
@dp.callback_query_handler(lambda c: c.data == "user_settings_change_lib")
...
# ---------- Wallet Handlers ----------
@dp.message_handler(lambda message: message.text == "💳 My Wallet", state="*")
async def wallet_menu(message: types.Message, state: FSMContext):
    ...
    await WalletStates.main.set()

@dp.message_handler(state=WalletStates.main)
async def wallet_menu_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == "🔙 Main Menu":
        ...
        await WalletStates.withdrawal_amount.set()
    else:
        await message.answer("Unknown option. Use buttons.")

@dp.message_handler(state=WalletStates.withdrawal_amount)
async def withdrawal_amount(message: types.Message, state: FSMContext):
    ...
    await WalletStates.withdrawal_method.set()

@dp.message_handler(state=WalletStates.withdrawal_method)
async def withdrawal_method(message: types.Message, state: FSMContext):
    ...
    await WalletStates.withdrawal_details.set()

@dp.message_handler(state=WalletStates.withdrawal_details)
...
# ---------- Countries & Pricing ----------
@dp.message_handler(lambda message: message.text == "🌍 Countries & Pricing", state="*")
...
# ---------- Claim Callbacks ----------
@dp.callback_query_handler(lambda c: c.data.startswith("claim_user_"))
async def claim_user_callback(callback_query: types.CallbackQuery):
    ...
    acc = await get_account(phone)
    if not acc:
        await bot.answer_callback_query(callback_query.id, text="Account not found.", show_alert=True)
        return
    if acc['added_by'] != user_id:
        await bot.answer_callback_query(callback_query.id, text="You are not the owner of this account.", show_alert=True)
        return
    if acc.get('claim_status') == 'claimed':
        await bot.answer_callback_query(callback_query.id, text="Already claimed!", show_alert=True)
        try:
    ...
    # Show processing
    await bot.answer_callback_query(callback_query.id, text="Processing claim...")
    ...
    if success:
        ...
        await bot.answer_callback_query(callback_query.id, text=msg[:200], show_alert=True)
        await bot.send_message(user_id, f"❌ {msg}")

@dp.callback_query_handler(lambda c: c.data.startswith("claim_admin_"))
...
# ---------- Admin Claim All ----------
@dp.message_handler(lambda message: message.text == "📋 Claim Accounts")
...
@dp.callback_query_handler(lambda c: c.data == "claim_all_force")
...
# ---------- Account Info ----------
@dp.message_handler(lambda message: message.text == "ℹ️ Account Info")
...
# ---------- Finance Management ----------
@dp.message_handler(lambda message: message.text == "💰 Finance Management")
...
@dp.message_handler(lambda message: message.text == "💰 All Balances")
...
@dp.message_handler(lambda message: message.text == "➕ Add Balance")
...
@dp.message_handler(lambda message: message.text == "➖ Deduct Balance")
...
@dp.message_handler(lambda message: message.text == "⏳ Pending Withdrawals")
...
@dp.message_handler(lambda message: message.text == "📤 Export Finance CSV")
async def export_finance_csv(message: types.Message):
    ...
    output.seek(0)
    await bot.send_document(message.from_user.id, types.InputFile(output, filename="finance.csv"))

# ---------- Country Management ----------
@dp.message_handler(lambda message: message.text == "🌍 Country Management")
...
@dp.callback_query_handler(lambda c: c.data == "add_country_prompt")
async def add_country_prompt_cb(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await AdminStates.waiting_for_country_code.set()

@dp.message_handler(state=AdminStates.waiting_for_country_code)
...
@dp.callback_query_handler(lambda c: c.data == "config_country_prompt")
...
@dp.callback_query_handler(lambda c: c.data.startswith("config_country_"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("add_api_"))
async def add_api_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await AdminStates.waiting_for_add_api_id.set()

@dp.message_handler(state=AdminStates.waiting_for_add_api_id)
async def process_add_api_id(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_add_api_hash.set()

@dp.message_handler(state=AdminStates.waiting_for_add_api_hash)
...
@dp.callback_query_handler(lambda c: c.data.startswith("remove_api_"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("remove_api_confirm_"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("add_proxy_"))
async def add_proxy_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await AdminStates.waiting_for_add_proxy.set()

@dp.message_handler(state=AdminStates.waiting_for_add_proxy)
...
@dp.callback_query_handler(lambda c: c.data.startswith("remove_proxy_") and not c.data.startswith("remove_proxy_confirm_"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("remove_proxy_confirm_"))
...
async def set_country_config_cb(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if action == "price":
        ...
        await AdminStates.waiting_for_country_price.set()
        await state.update_data(country_code=code)
        ...
        await AdminStates.waiting_for_country_capacity.set()
        await state.update_data(country_code=code)
    elif action == "proxy":
        ...
        await AdminStates.waiting_for_country_proxy.set()
        await state.update_data(country_code=code)
    elif action == "timer":
        ...
        await AdminStates.waiting_for_country_timer.set()
        await state.update_data(country_code=code)

@dp.message_handler(state=AdminStates.waiting_for_country_price)
...
@dp.message_handler(state=AdminStates.waiting_for_country_capacity)
...
@dp.message_handler(state=AdminStates.waiting_for_country_proxy)
...
@dp.message_handler(state=AdminStates.waiting_for_country_timer)
...
# ---------- Toggle Country ----------
@dp.callback_query_handler(lambda c: c.data.startswith("toggle_country_"))
...
# ---------- Search Accounts ----------
@dp.message_handler(lambda message: message.text == "🔎 Search Accounts")
async def search_accounts_prompt(message: types.Message, state: FSMContext):
    ...
    await SearchStates.waiting_for_search_query.set()

@dp.message_handler(state=SearchStates.waiting_for_search_query)
...
# ---------- Bulk Delete ----------
@dp.message_handler(lambda message: message.text == "📁 Bulk Delete")
async def bulk_delete_prompt(message: types.Message, state: FSMContext):
    ...
    await message.answer(text, parse_mode="Markdown")
    await BulkDeleteStates.waiting_for_phone_list.set()

@dp.message_handler(state=BulkDeleteStates.waiting_for_phone_list)
...
# ---------- Export CSV ----------
@dp.message_handler(lambda message: message.text == "📤 Export CSV")
async def export_accounts_csv(message: types.Message):
    ...
    output.seek(0)
    await bot.send_document(message.from_user.id, types.InputFile(output, filename="accounts.csv"))

# ---------- Submit Account (modified to store API) ----------
@dp.message_handler(lambda message: message.text in ["➕ Create Login", "➕ Add Account"])
async def create_login_start(message: types.Message, state: FSMContext):
    ...
    if await get_user_auto_add_mode(user_id):
        ...
        await LoginStates.entering_phone.set()
    else:
        await message.answer("Which library?", reply_markup=await lib_keyboard())
        await LoginStates.choosing_lib.set()

@dp.message_handler(state=LoginStates.choosing_lib)
...
@dp.message_handler(state=LoginStates.entering_phone)
async def phone_entered(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await LoginStates.entering_code.set()
    except Exception as e:
...
@dp.message_handler(state=LoginStates.entering_code)
async def code_entered(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if await get_user_auto_add_mode(user_id):
            ...
            login_data.pop(user_id, None)
            login_data[user_id] = {'lib': lib}
            await LoginStates.waiting_for_phone_continuous.set()
        else:
            login_data.pop(user_id, None)
            await state.finish()
        ...
        await LoginStates.entering_password.set()
    except Exception as e:
...
@dp.message_handler(state=LoginStates.entering_password)
...
@dp.message_handler(state=LoginStates.waiting_for_phone_continuous)
...
@dp.message_handler(lambda message: message.text == "📥 Import Session")
async def import_session_prompt(message: types.Message, state: FSMContext):
    ...
    await ImportStates.waiting_for_import_data.set()

@dp.message_handler(state=ImportStates.waiting_for_import_data, content_types=types.ContentType.DOCUMENT)
...
@dp.message_handler(state=ImportStates.waiting_for_import_data)
async def import_session_text(message: types.Message, state: FSMContext):
    ...
    await message.answer("📱 Enter phone number (with +):", reply_markup=cancel_keyboard())
    await ImportStates.waiting_for_phone.set()
...
@dp.message_handler(state=ImportStates.waiting_for_phone)
async def import_phone(message: types.Message, state: FSMContext):
    ...
    await ImportStates.waiting_for_lib.set()

@dp.message_handler(state=ImportStates.waiting_for_lib)
...
@dp.message_handler(lambda message: message.text == "📨 Read Messages")
...
@dp.message_handler(lambda message: message.text == "🗑 Delete Session")
...
@dp.message_handler(lambda message: message.text == "🚪 Revoke Session")
...
@dp.message_handler(lambda message: message.text == "📤 Export Files")
...
@dp.message_handler(lambda message: message.text == "📦 Bulk Export")
...
async def process_bulk_export(callback_query: types.CallbackQuery):
    ...
    zip_buffer.seek(0)
    zip_file_obj = types.InputFile(zip_buffer, filename=f"sessions_{code}.zip")
...
@dp.message_handler(lambda message: message.text == "🔐 Set 2FA (All)")
async def set_2fa_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_2fa_password.set()

@dp.message_handler(state=AdminStates.waiting_for_2fa_password)
...
@dp.message_handler(lambda message: message.text == "⚙️ Proxy Settings")
...
@dp.message_handler(lambda message: message.text == "➕ Set New Proxy")
async def add_proxy_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_proxy_host.set()

@dp.message_handler(state=AdminStates.waiting_for_proxy_host)
async def process_proxy_host(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_proxy_port.set()

@dp.message_handler(state=AdminStates.waiting_for_proxy_port)
async def process_proxy_port(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_proxy_username.set()

@dp.message_handler(state=AdminStates.waiting_for_proxy_username)
async def process_proxy_username(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_proxy_password.set()

@dp.message_handler(state=AdminStates.waiting_for_proxy_password)
...
@dp.message_handler(lambda message: message.text == "🔄 Try Again", state="*")
...
@dp.message_handler(lambda message: message.text == "🗑 Remove Proxy")
...
@dp.message_handler(lambda message: message.text == "🔍 Health Check")
...
@dp.message_handler(lambda message: message.text == "🤖 Check Spam (All)")
...
@dp.message_handler(lambda message: message.text == "🔍 Check Proxies")
...
# ---------- Add/Remove Admin ----------
@dp.message_handler(lambda message: message.text == "➕ Add Admin")
async def add_admin_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_add_admin_id.set()

@dp.message_handler(state=AdminStates.waiting_for_add_admin_id)
...
@dp.message_handler(lambda message: message.text == "➖ Remove Admin")
async def remove_admin_prompt(message: types.Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=cancel_keyboard(), parse_mode="Markdown")
    await RemoveAdminStates.waiting_for_remove_admin_id.set()

@dp.message_handler(state=RemoveAdminStates.waiting_for_remove_admin_id)
...
@dp.message_handler(lambda message: message.text == "📋 List Admins")
...
@dp.message_handler(lambda message: message.text == "📢 Broadcast to Admins")
async def broadcast_admins_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_broadcast_admins_message.set()

@dp.message_handler(state=AdminStates.waiting_for_broadcast_admins_message)
async def process_broadcast_admins(message: types.Message, state: FSMContext):
    ...
    if last_broadcast[message.from_user.id]:
        ...
        await AdminStates.waiting_for_broadcast_pin_confirm.set()
    else:
        await state.finish()

@dp.callback_query_handler(lambda c: c.data == "pin_broadcast", state=AdminStates.waiting_for_broadcast_pin_confirm)
...
@dp.callback_query_handler(lambda c: c.data == "dont_pin_broadcast", state=AdminStates.waiting_for_broadcast_pin_confirm)
...
@dp.message_handler(lambda message: message.text == "📢 Broadcast to All Users")
async def broadcast_users_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_broadcast_users_message.set()

@dp.message_handler(state=AdminStates.waiting_for_broadcast_users_message)
...
# ---------- User List ----------
@dp.message_handler(lambda message: message.text == "👥 User List")
...
@dp.message_handler(lambda message: message.text == "📨 Send Message")
async def send_message_prompt_user_id(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_send_user_id.set()

@dp.message_handler(state=AdminStates.waiting_for_send_user_id)
async def process_send_user_id(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_send_message.set()

@dp.message_handler(state=AdminStates.waiting_for_send_message)
...
# ---------- Reset User Account ----------
@dp.message_handler(lambda message: message.text == "🔄 Reset User Account")
async def reset_user_account_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_reset_user_id.set()

@dp.message_handler(state=AdminStates.waiting_for_reset_user_id)
...
# ---------- Set Admin Group ----------
@dp.message_handler(lambda message: message.text == "🏢 Set Admin Group")
...
@dp.message_handler(lambda message: message.text == "📢 Set User Join Group")
async def set_user_join_group_prompt(message: types.Message, state: FSMContext):
    ...
    if current:
        text += f"\nCurrent: `{safe_md(str(current))}`"
    await message.answer(text, reply_markup=cancel_keyboard(), parse_mode="Markdown")
    await AdminStates.waiting_for_user_join_group_id.set()

@dp.message_handler(state=AdminStates.waiting_for_group_id)
...
@dp.message_handler(state=AdminStates.waiting_for_user_join_group_id)
...
# ---------- Reset All Data ----------
@dp.message_handler(lambda message: message.text == "🗑 Reset All Data")
async def reset_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_reset_confirmation.set()

@dp.message_handler(state=AdminStates.waiting_for_reset_confirmation)
...
# ---------- Toggle Libraries ----------
@dp.message_handler(lambda message: message.text == "⚡ Toggle Libraries")
...
@dp.callback_query_handler(lambda c: c.data.startswith("toggle_lib_"))
...
# ---------- Set Support ID ----------
@dp.message_handler(lambda message: message.text == "📞 Set Support ID")
async def set_support_prompt(message: types.Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_support_id.set()

@dp.message_handler(state=AdminStates.waiting_for_support_id)
...
# ---------- Restart Bot ----------
@dp.message_handler(lambda message: message.text == "🔄 Restart Bot")
...
# ---------- Account List ----------
@dp.message_handler(lambda message: message.text in ["📋 Accounts", "📋 Account List"])
...
@dp.message_handler(lambda message: message.text == "📂 Session Files")
...
@dp.message_handler(lambda message: message.text == "📦 Download All Sessions ZIP")
async def download_all_sessions_zip(message: types.Message):
    ...
    if count == 0:
        ...
        return
    zip_buffer.seek(0)
    zip_file = types.InputFile(zip_buffer, filename=f"Account_Management_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.zip")
    await bot.send_document(
    ...
    if group_id:
        try:
            zip_buffer.seek(0)
            zip_file2 = types.InputFile(zip_buffer, filename=f"Account_Management_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.zip")
...
@dp.message_handler(lambda message: message.text == "📋 List Recent Sessions")
...
@dp.message_handler(lambda message: message.text == "🗑 Clear Old Sessions (7d+)")
...
@dp.message_handler(lambda message: message.text == "🌐 Toggle Bot")
    ...
    for user in users:
        try:
            if await is_admin(user['user_id']):
                await bot.send_message(user['user_id'], "🟢 **Bot is now online!**", reply_markup=admin_panel_keyboard(), parse_mode="Markdown")
            else:
                await bot.send_message(user['user_id'], "🟢 **Bot is now online!**", reply_markup=await main_menu_keyboard(user['user_id']), parse_mode="Markdown")
            await asyncio.sleep(0.05)
        except Exception as e:
